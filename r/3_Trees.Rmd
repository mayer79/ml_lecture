...
2. Split the diamonds data set in train/valid/test sets. Fit a regression tree on the training data, using the four "C" variables as covariables. 
3. Use performance on the validaiton data to optimize 1-2 of the parameters found in 1.
...
0. In above example: How large is the out-of-bag RMSE? Is it similar to the test RMSE?
1. In above example, replace carat by its logarithm. Do the results change compared to the example without logs?
2. Instead of taking logarithms of carat, model log(price). Interpret the results and compare with the model without logs.
3. Fit a random forest on the titanic training data. Make sure to fit a probabilistic classification, not a regression.
...
We will revisit the titanic survival model and use XGBoost to fit it - following the four step strategy to select parameters described above.
...
```
Now, the model is ready to be inspected by evaluating its test performance, by looking at gain importance and a couple of partial dependence plots.
...
1. Study the documentation of XGBoost to figure out how to make the model monotonically increasing in carat. Test your insights by modifying Step 4 in the code above. How does the partial dependence plot for carat look now?
2. Use the code in above example to "play through" the same situation but with log(price) as response.

Note: The titanic data set seems a bit too small to attack with tools like XGBoost, although it is possible.
