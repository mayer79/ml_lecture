...
- **Regularization parameters:** Depending on the implementation, additional parameters are
  ...
  Reasonable regularization parameters are chosen by randomized or grid search CV. Usually, it takes a couple of iterations until the range of the parameter values have been set appropriately.
...
1. Study the documentation of XGBoost to figure out how to make the model monotonically increasing in carat. Test your insights without rerunning the grid search in our last example, i.e. just be refitting the final model. How does the partial dependence plot for `carat` look now?
