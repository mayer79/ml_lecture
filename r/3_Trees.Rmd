...
- Step 4: Choose the best parameter combination from Step 3 and refit to get the final model.
...
if (tune) {
 ...
 #   tree_method = "hist",   # when data is large
  ...
  coarse_grid <- fit_grid(dtrain, coarse_grid, file = "gridsearch/diamonds_xgb_coarse.rds")
  ...
  fine_grid <- fit_grid(dtrain, fine_grid, file = "gridsearch/diamonds_xgb_fine.rds")
  fine_grid[order(fine_grid$score), ]
}

# STEP 4: Fit with best parameter combination
fine_grid <- readRDS("gridsearch/diamonds_xgb_fine.rds")
