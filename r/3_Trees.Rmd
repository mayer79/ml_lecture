...
We will use the `dataCar` data set to predict the claim probability with a decision tree. As features, we use `veh_value`, `veh_body`, `veh_age`, `gender`, and `agecat`.
...
- **Diversification:** Single trees in a random forest are usually very deep and overfit on the training data. It is the diversity across trees that produces a good and stable model, just with a well-diversified stock portfolio. 
...
- **Parameter tuning:** Random forests offer many tuning parameters. Since the results do not depend too much on their choice, untuned random forests are often great benchmark models.

## Example: random forest

Let us now fit a random forest for diamond prices with default parameters and 500 trees. 80% of the data is used for training, the other 20% we use for evaluating the performance. We use stratified splitting on the response variable. 
...
Modern implementations of such *gradient boosted trees* are [XGBoost](https://xgboost.readthedocs.io/en/latest/), [LightGBM](https://lightgbm.readthedocs.io/en/latest/) and [CatBoost](https://catboost.ai/). These are the predominant algorithms in ML competitions on tabular data, check [this comparison](https://github.com/mayer79/gradient_boosting_comparison) for differences with a screenshot as per Sept. 07, 2020:
...
Reasonable regularization parameters are chosen by randomized or grid search CV. Usually, this takes a couple of iterations.
...
We will use XGBoost to fit diamond prices using MSE as loss function and evaluation metric.

The basic strategy is as follows:

1. Using default regularization, set the learning rate to get reasonable number of trees with CV-based early stopping.
...
if (tune) {
  # Use default parameters to get learning rate with suitable number of rounds
  params <- list(
    learning_rate = 0.05,
  ...
  cvm   # -> a lr of 5% provides about 160 trees
 ...
 # Grid search or randomized search if grid is too large
  ...
  for (i in seq_len(grid_size)) {
    ...
    # Save grid to surviva hard crashs
...
rmse(diamonds$price[ix$test], pred)       # 541 USD
...
1. Study the documentation of XGBoost to figure out how to make the model monotonically increasing in carat. Test your insights without rerunning the grid search in our last example. How does the partial dependence plot for `carat` look now?
