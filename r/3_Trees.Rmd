...
prp(
  fit, type = 2, extra = 7, shadow.col = "gray",
...
options(ranger.num.threads = 8)  # Adapt to your CPU
...
Modern variants of such *gradient boosted trees* are [XGBoost](https://xgboost.readthedocs.io/en/latest/), [LightGBM](https://lightgbm.readthedocs.io/en/latest/) and [CatBoost](https://catboost.ai/). These are the predominant algorithms in ML competitions on tabular data, check [this comparison](https://github.com/mayer79/gradient_boosting_comparison) for differences with a screenshot as per Feb. 07, 2025:
...
fit <- xgb.train(
  ...
  data = dtrain,
  print_every_n = 50,
  nrounds = cvm$early_stop$best_iteration
)

# Test performance
rmse(y_test, predict(fit, data.matrix(X_test)))  # 544.1
```
...
if (tune) {
  ...
  cvm   # -> a lr of 0.02 provides about 430 trees, which is a convenient amount
...
rmse(y_test, pred)                                       # 545.7
r_squared(y_test, pred, reference_mean = mean(y_train))  # 0.982
