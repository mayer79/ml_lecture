...
Note on **evaluation metrics**: While loss functions are used by the algorithm to *fit* the model, an evaluation metric is a function used to monitor performance, to tune parameters and to select models. Ideally, it is consistent with the loss function of the specific algorithm (for instance RMSE as metric and squared error as loss), but there might be exceptions.
