...
$$
To quantify the size of the typical prediction error on the same scale as $Y$, we can take the square-root of the MSE and study the *root-mean-squared error* (RMSE). 
...
$$
  E(Y) = \alpha
$$
without any covariables, just with an intercept. Put differently, the R-squared measures the proportion of variability of $Y$ explained by the covariables.
...
$$
and normal distribution of $\varepsilon$ guarantee optimality of the least-squares estimator $\hat \beta_0, \dots, \hat\beta_m$ and the correctness of inferential statistics (standard errors, p values, confidence intervals). In that case, we talk of the *normal linear model*.
...
- **Slope:** Adding covariable `color` has changed the slope of `carat` from 7756 in the simple linear regression to 8067. This is the effect of `carat` *adjusted* for `color`.
...
We will briefly look into these.

### Non-linear terms

Important numeric covariables might be represented by more than just one parameter (= linear slope) to allow for more flexible and non-linear associations to the response. Adding a squared term e.g. allows for curvature, adding multiple polynomial terms can approximate any smooth relationship. An alternative to polynomial terms are regression *splines*, i.e. piecewise polynomials.

For our diamonds data set, polynomial terms up to degree three for `carat` would look like this:
...
- In the dense part (carats up to 2), the cubic polynomial seems to provide better results than a simple linear regression.
...
- Adding interaction terms make model interpretation difficult. They should thus be used with care.
...
Let us now fit a linear regression for diamond prices with the two covariables `carat` and `color`. How would a model with interaction between the two variables look compared to the model without?
...
- In the model with interactions (image right), we get different slopes and intercepts per color, just as if we would have fitted a simple linear regression *per color*. The larger the diamonds, the larger the color effects.
...
Not surprisingly, its coefficient explains how the *transformed* variable $X$ acts on the expected response. With The logarithm is a transformation that even allows interpretation regarding the untransformed $X$. In the model equation
$$
  E(Y\mid X = x) = \alpha + \beta \log(x),
$$
we can say: A 1% increase in $X$ leads to an increase in $E(Y)$ of about $\beta/100$:
$$
  E(Y\mid X = 101\% \cdot x) - E(Y\mid X = x) = \alpha + \beta \log (1.01 \cdot x) - \alpha - \beta \log(x) = \beta \log(1.01) \approx \beta/100.
$$
...
What would our simple linear regression give with logarithmic carat as single covariable?
...
- Indeed, we have fitted a logarithmic relationship between carat and price. The scatterplot reveals that this does not make much sense. The model looks wrong. Shouldn't we better take the logarithm of price?
...
We have seen that taking logarithms not only reduces outlier effects in covariables but they also allow to think in relative changes. What happens if we consider a log response? The model of a simple linear regression would be
$$
  E(\log{Y} \mid X = x) = \alpha + \beta x.
$$  
**Claim:** The effect $\beta$ tells us by how much we can expect $Y$ to change. Thus, a logarithmic response leads to a *multiplicative* instead of an *additive* one.
...
$$
  E(Y\mid X = x+1) - E(Y\mid X = x) = e^{\alpha + \beta (x+1)} - e^{\alpha + \beta x} = e^{\alpha + \beta x}(e^\beta - 1) = E(Y\mid X = x)(e^\beta - 1).
$$
...
Since expectations and logarithms cannot be swapped, the calculation is not 100% correct. As a consequence, predictions on the scale of $Y$ are too small. In the section on GLMs, we will see a cleaner method to create multiplicative models.
...
data.frame(carat = seq(0.3, 2.5, by = 0.1)) %>% 
...
- An increase in carat of 0.1 is associated with a log(price) increase of 0.197.
- An increase in carat of 0.1 is associated with a price increase of about 20%.
- The model looks okay until 2 carat. For larger diamonds, the model is heavily biased.
- Predictions are obtained by exponentiating the result of the linear formula.
...
- **General impression:** The model looks quite realistic, both on backtransformed scale and linear scale. There are no obvious model biases visible.
...
- **R-squared:** About 93% of the variability in log(price) can be explained by log(carat).
- **RMSE:** Typical prediction errors are in the range of 26%.

## Example: diamonds improved

To end the section on linear regression, we will start from the log-log-example above and improve it by adding `color`, `cut` and `clarity`.
...
- **General:** In contrast to the descriptive analysis with wrong price effects of `color`, `cut` and `clarity`, all effects of the multiple linear regression look plausible.
...
- **R-squared:** About 98% of the variability in log-prices can be explained by our four covariables. Adding the three categorical covariables has considerably improved the precision model.
...
This section covers the generalized linear model. It was introduced in 1972 by Nelder & Wedderburn [2]. 
...
- Parameters of a GLM are estimated by Maximum-Likelihood. This amounts to minimizing a distribution specific objective or loss function called *deviance*. 
- The deviance of the normal distribution equals the MSE. Deviance plays the same role for GLMs as MSE for linear regression.
- The deviance of logistic regression is equivalent to cross-entropy and log loss.
- The logit function is defined as 
$$
  \text{logit}(p) = \log (\text{Odds}(p)) = \log\left(\frac{p}{1-p}\right)
$$
It maps probabilities to the real line. The inverse logit ("sigmoidal transformation" or "logistic function") reverts this: It maps real values to the range from 0 to 1. In the context of GLMs, $p = \text{Prob}(Y = 1) = E(Y)$.
...
**Outlook:** The loss functions used in the context of GLMs are used one-to-one as objective functions for other ML methods like gradient boosting or neural networks. There, the "appropriate" loss function is chosen from the context. E.g. if the response is binary, one typically selects binary cross-entropy as the optimization criterion.
...
In these examples, one could think of transforming the response variable e.g. by a logarithm to produce a multiplicative model. This, however fails due to the simple fact that logarithms of 0 are not possible.
