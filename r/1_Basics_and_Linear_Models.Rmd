...
The more time we invest in the descriptive analysis, the more we learn about the data, which is essential for successful analysis. Besides getting used to the data, we might 
...
- The relationship between price and carat seems quite strong and positive, which makes sense. On log-log scale, it looks quite linear.
...
The main assumption of linear regression is a **correctly specified model equation** in the sense that predictions are not systematically too high or too small for certain values of the covariables. In a simple regression setting, this means that the points in the scatterplot are located *around* the regression line for all covariable values. For a multiple linear regression, this translates to the empirical condition that residuals (differences between observed and fitted response) do not show bias if plotted against covariable values.
...
For small diamonds, the predictions of our simple linear regression are systematically too low. This indicates a wrongly specified model. We will see ways to fix this.
...
Like many other ML algorithms, linear regression cannot deal with missing values. Rows with missing response can be safely dropped, while missing values in covariables should usually be dealt with. The simplest (often too naive) approach is to fill missing values with a typical value such as the mean or the most frequent value.

### Outliers

Gross outliers in covariables can distort the result of the linear regression. Do not delete them, but try to reduce their effect by taking logarithms or by using more robust regression techniques. Outliers in the response can be problematic as well, especially for inferential statistics.
...
Strong collinearity can be detected by looking at correlations across (numeric) covariables. It is mainly a problem when interpreting effects or for statistical inference of effects. Predictions or other "global" model characteristics like the R-squared are not affected.
...
- **Integer encoding:** Ordinal categorical covariables are sometimes integer encoded for simplicity, i.e. each category is represented by an integer number. 
...
Important numeric covariables might be represented by more than just one parameter (= linear slope) to allow for more flexible and non-linear associations to the response. Adding a squared term e.g. allows for curvature, adding enough multiple polynomial terms can approximate any smooth relationship. 

For example, the model equation for a cubic regression is
$$
  E(Y) = \beta_0 + \beta_1 x + \beta_2 x^2 + \beta_3x^3.
$$
An alternative to polynomial terms are regression *splines*, i.e. piecewise polynomials. Using non-linear terms makes interpretation of regression coefficients difficult. An option is to study predictions while sliding the covariable values over its range (systematic predictions).
...
- Extrapolation to diamonds above 2 carat provides catastrophic results. Thus, be cautious if extrapolation is required.
...
- Back-transformed, this amounts to saying that a 1% increase in `carat` is associated with a price increase of about 5836/100 = 60 CHF.
...
Assume for a moment that we can swap taking expectations and log (disclaimer: we cannot). In that case, the model would be
...
- **General impression:** The model looks quite realistic, both on back-transformed scale and the log-log scale. There are no obvious model biases visible.
...
To end the section on linear regression, we extend the log-log-example above by adding `color`, `cut` and `clarity` as categorical covariables.
...
As alternative to the multiple linear regression on diamond prices with logarithmic price and logarithmic carat, consider the same model without logarithms. Interpret the output of the model. Does it make sense from practical perspective?
...
$$
where $Y$ conditional on the covariables belongs to the exponential dispersion family and $g$ is a transformation. 
...
- binary responses (e.g. insurance claim yes/no, success yes/no, fraud yes/no), 
- count responses (e.g. number of insurance claims, number of adverse events),
- right-skewed responses (e.g. time durations, claim heights, prices).
...
GLMs deal with such problems by taking a suitable link function like the logarithm. At least for the first two examples, this could not be done by linear regression with log response because of zeros.

Further advantages of the GLM over the linear regression are:

- Predictions are on the right scale, e.g. probabilities of a binary response are between 0 and 1 if we use the logit link. With linear regression, they could be outside [0, 1].
...
- **Identity link:** As with linear regression: "A one-point increase in $X$ is associated with a $\beta$ increase in $g(E(Y))$, keeping everything else fixed". 
- **Log link:** As with linear regression with log response: "A one-point increase in $X$ is associated with a relative increase in $E(Y)$ of $e^{\beta}-1 \approx \beta \cdot 100\%$. The derivation is exactly as we have seen for linear regression, except that we start with $\log(E(Y))$ instead of $E(\log(Y))$, making the calculations mathematically sound. Using a GLM with log link is thus the cleaner way to produce a multiplicative model for $E(Y)$ than to log transform the response in a linear regression.
- **Logit link:** Logistic regression uses the logit link
$$
  \text{logit}(p) = \log (\text{odds}(p)) = \log\left(\frac{p}{1-p}\right).
$$
It maps probabilities to the real line. The inverse logit ("sigmoidal transformation" or "logistic function") reverts this: It maps real values to the range from 0 to 1. Odds, i.e. the ratio of $p$ to $1-p$, is a concept borrowed from gambling: The probability to get a "6" is 1/6. Correspondingly, the *odds* to get a "6" is 1:5 = 0.2. By definition, logistic regression is a multiplicative model for the odds of getting a 1. Correspondingly, the coefficients $e^\beta - 1 \approx \beta$ are called *odds ratios*. We do not interpret the coefficients on the original probability scale.

![](../figs/log_odds.png)

## Parameter estimation and deviance

Parameters of a GLM are estimated by Maximum-Likelihood theory. This amounts to minimizing a distribution specific objective or loss function called *deviance*, which is the sum of the loss per row (see table above), eventually weighted by case weights. For the normal linear model, deviance (divided by n) is equivalent to MSE. In fact, deviance plays the same role for GLMs as MSE for linear regression. It e.g. makes sense to use relative deviance improvement compared to an empty model as a relative performance measure.

Remark: The deviance of the logistic regression is equivalent to *cross-entropy* or *log loss*. 
...
- The deviance 26629 is only 0.5% smaller than the one of the null model 26768. The predictive performance of this model is thus low. This makes sense: having claims is a highly random event that cannot be predicted on individual scale.

## Example: logistic regression

In order to illustrate logistic regression, we will model the binary variable "claim yes=1/no=0" of the claims data by a logistic regression. Its logit link ensures that predicted probabilities are between 0 and 1 and that covariables act in a multiplicative way on the odds of having a claim.
