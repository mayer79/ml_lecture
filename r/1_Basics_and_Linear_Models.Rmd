...
- **Predictions:** Model predictions are made by using the fitted model equation $-2256 + 7756 \cdot \text{carat}$. For a diamond of size 0.3 carat, we get 71 USD. For 1.3 carat, we get $-2256 + 1.3 \cdot 7756 \approx 7827$. These values correspond to the values on the regression line.
...
Since algorithms usually only understand numbers, categorical variables are encoded by numbers. The standard approach is called **one-hot-encoding** (OHE) and works as follows: Each level $x_k$ of the categorical variable $X$ gets its own binary **dummy** variable $D_k = 1(X = x_k)$, indicating if $X$ has this particular value or not. In linear models, one of the dummy variables ($D_1$, say) is being dropped and its level is represented by the intercept. This variant of OHE is called **dummy coding**.
...
- Interpreting the regression coefficient $\beta_k$ of the dummy variable $D_k$ is nothing special: It tells us how much $E(Y)$ changes when switching from the reference category (the one without dummy) to category $k$.
...
Let us now extend the simple linear regression by adding dummy variables for the categorical covariable `color`.
...
- Each dummy variable of `color` has received its coefficient. Switching from the reference color "D" to "E" is associated with a price reduction of 94 USD.
- In contrast to the unintuitive descriptive result (worse colors tend to higher prices), we now see that worse colors are associated with lower prices. Adjusting for carat has solved the mistery. It seems that the diamond size has confounded the association between color and price.
...
This section covers the generalized linear model, introduced in 1972 by Nelder & Wedderburn [2]. 
