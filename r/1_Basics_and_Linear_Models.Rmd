...
3. A distribution of $Y$ conditional on the covariates. It implies the loss function to be minimized over the training sample.
...
Parameters of a GLM are estimated by Maximum-Likelihood. This amounts to minimizing a distribution specific objective criterion called (total) *deviance*, which is proportional to the sum of the losses (see table above) of all rows, eventually weighted by case weights. For the normal linear model, deviance (divided by n) is equivalent to MSE. In fact, deviance plays the same role for GLMs as the MSE for linear regression. It sometimes makes sense to consider as a relative performance measure the relative deviance improvement compared to an empty model. For the normal linear regression model, this equals the usual R-squared.

Remark: The unit deviance of the logistic regression equals twice the binary *cross-entropy* (= "log loss"). 
