...
If the association between two or more covariates is strong, their coefficients are difficult to interpret because the Ceteris Paribus clause is usually unnatural in such situations. E.g. in a house price model, it is unnatural to study the effect of an additional room, keeping living area fixed. This is even more problematic for causally dependent covariates: imagine a model with $x$ and $x^2$ as covariates. It would certainly not make sense to study the effect of $x$ while keeping $x^2$ fixed. 
...
Note: Perfectly collinear covariates (e.g. $X$ and $2X$) cannot be used for algorithmic reasons.
...
1. A linear function for the covariate effects, exactly as in linear regression.
...
3. A distribution of $Y$ conditional on the covariates.
