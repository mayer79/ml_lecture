...
- Interpreting the regression coefficient $\beta_k$ of the dummy variable $D_k$ is nothing special: It tells us how much $E(Y)$ changes when the dummy variable switches from 0 to 1. This amounts to switching from the reference category (the one without dummy) to category $k$.
- Ordinal categorical covariables are sometimes integer encoded for simplicity i.e. each category is represented by an integer number.
- In order to reduce overfitting, small categories are often combined to a level "Other" or are added to the largest category.
...
- Adding covariable `color` has changed the slope of `carat` from 7756 in the simple linear regression to 8067. This is the effect of `carat` *adjusted* for `color`.
- Each dummy variable of `color` has received its own coefficient. Switching from the reference color "D" to "E" is associated with a price reduction of 94 USD.
...
Linear regression is flexible regarding *how* covariables are represented in the linear equation. Possibilities include
...
- Interaction terms mean more parameters, thus there is a danger of overfitting.
- Finding the right interaction terms without introducing overfitting is impossible.
...
Let us now fit a linear regression for diamond prices with the two covariables `carat` and `color`. How would a model with interaction between the two variables look compared to the model without interactions from our example on dummy coding?
...
- collinear covariables are decorrelated by forming ratios,
- logarithms neutralize gross outliers.

Model coefficients of course explain how the *transformed* variable $X$ acts on the expected response. In this respect, logarithms are special: In a model equation of
$$
  E(Y) = \alpha + \beta \log(X),
$$
we can say: A 1% increase in $X$ leads to an increase in $E(Y)$ of about $\beta/100$:
$$
  E(Y\mid X = 1.01 \cdot x) - E(Y\mid X = x) = \alpha + \beta \log (1.01 \cdot x) - \alpha - \beta \log(x) = \beta \log(1.01) \approx \beta/100.
$$
...
- Indeed, we have fitted a logarithmic relationship between carat and price. The scatterplot reveals that this does not make much sense. The model looks wrong.
