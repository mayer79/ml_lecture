...
The basic linear model has many extensions, e.g.
...
- penalized regression, a group of methods to fight overfitting,
- neural networks that automatically learn interactions and non-linearities,
- **the generalized linear model (GLM)** that e.g. allows to model binary response variables in a natural way.

Here, we will introduce the generalized linear model.




- We now switch to a binary classification situation. We use binary logistic regression to model claim (y/n) 
- Poisson
- Excercise Gamma?
...
fit <- glm(clm ~ veh_value + veh_body + veh_age + gender + area + agecat, 
           data = dataCar[ix$train, ], family = binomial)
summary(fit)
