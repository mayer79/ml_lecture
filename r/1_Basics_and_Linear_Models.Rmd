...
In this chapter, we provide basic notatations and revisit two classic but fundamental ML algorithms: linear regression and the generalized linear model. Both are *linear models*.
...
- **categorical:** The values of a categorical variable are categories, e.g. house types, claim types or colors. Depending on whether categories follow a natural order, we talk of *ordered* or *unordered* categoricals. Categories can be encoded by numbers. That does not make the variable numeric as it does not make sense to calculate means.
...
A modeling task always starts with a descriptive analysis of the data. This typically involves numeric and/or graphical summary of each variable and the most relevant variable pairs, e.g. 
...
- identify data errors and problematic outliers, 
...
p <- ggplot(diamonds, aes(y = price, x = carat)) +
  geom_point(alpha = 0.2, shape = ".")

p + ggtitle("Price against carat")

p + ggtitle("Price against carat - logarithmic scales") +
  scale_y_log10() + 
  scale_x_log10()
...
- On (natural) logarithmic scale, the relationship between price and carat seems quite linear. We will use this insight later.
- The worst categories of `color`, `cut`, and `clarity` are rare.
- Prices tend to be lower for nice colors. This is unintuitive and might be explained by the fact that large diamonds have lower quality than small ones. This would mean that carat is a *confounder* of the relationship between color an price. More on this later.
...
Usually, we are interested in modeling a specific property of $y$, usually its expection $E$ (= theoretic mean). In that case, we can make above approximate relationship more explicit by writing down the *model equation*
$$
  E(y) = f(X).
$$
...
$$
It relates the regressors $X_1, \dots, X_m$ to the expected response $E(y)$ by *linear* formula in the parameter vector $\beta = (\beta_1, \dots, \beta_m)$. $X_1$ is an artificial variable with value 1 used for calibration. It is called *intercept*. The parameter $\beta_i$ tells us by how much $y$ is expected to change when $X_i$ is increased by 1, **keeping all other predictors fixed** ("Ceteris Paribus"):
$$
...
- The model equation stresses the fact that our interest is not in modeling the full statistical distribution of the response but rather just one aspect, namely its expectation.
...
$$
across our $n$ observations. $y_i$ and $X^{(i)}$ are the observed response and regressor vector of observation $1 \le i \le n$.

The optimal $\hat f$ resp. $\hat \beta$ is found analytically by the famous *least-squares* method with solution
$$
  \hat \beta = (X'X)^{-1}X'y.
$$
...
- In the equation $E(\text{price}) = \alpha + \beta \cdot \text{carat}$, the intercept $\alpha$ is estimated by $\hat \alpha = -2256$. Strictly speaking, this means that a 0 carat diamond costs -2256 USD, which is nonsensical but required for prediction.
- The slope $\beta$ is estimated by $\hat \beta = 7756$ USD. It means that a 1 carat increase goes along with an average increase in price of 7756 USD. Similarly, we could say that a 0.1 increase in carat is associated with an increase in the price of 775.6 USD.
...
- The R-squared is the proportion of variability explained by the covariables. Here, about 85% of the price variability can be explained by the variability in carat. The remaining 15% is yet unexplained.
...
Gross outliers in covariables can distort the result of the linear regression. Do not delete them, but try to reduce their effect by taking logarithms or by using more robust regression techniques.

### Overfitting

If too many parameters are fitted in relation to the number of observations, the resulting model might look good but would not generalize well to new data. This is called overfitting. A small amound of it is unproblematic but do not fit a model with $m=100$ parameters on a data set with just $n=200$ rows. The resulting model would be garbage. A $n/m$ ratio of 50-100 is usually safe for stable estimation of parameters.
...
- **Dummy coding:** Categoricals are usually dummy coded. This means that each category is represented by its own binary variable ("am I in this category yes/no?"). One of the dummy variables is already represented by the intercept and is thus dropped. This is often done implicitly by the software. For some ML algorithms, dropping the redundant dummy variable is not necessary/recommended. In that case we speak of "one-hot-encoding". Rare categories can be lumped together in order to not estimate too many parameters and to reduce overfitting.  
...
Here, we will introduce the generalized linear model. Its model equation is
$$
  g(E(Y)) = X\beta
$$
resp.
...
- $X\beta$ is a linear function $\beta_0 + \beta_1 x_1 \dots \beta_m x_m$ just like in linear regression to represent covariable effects,
- the link-function $g$ maps $E(Y)$ to the real line, i.e. to the scale of $X\beta$, and
- $Y$ conditional on $X$ follows a distribution in the exponential dispersion family.

The most typical choices are


| Regression type | Distribution | Natural link| Typical link  |
|-----------------|--------------|-------------|---------------|
| Linear          | Normal       | Identity    | Identity      |
| Binary          | Binary       | logit       | logit         |
| Count           | Poisson      | log         | log           |
| Log-linear      | Gamma        | 1/x         | log           |
| multinomial     | Multinomial  | mlogit      | mlogit        |

The first type (linear) corresponds to the linear regression model covered in the last section.

The logit function is defined as 
$$
  \text{logit}(p) = \log (\text{odds}(p)) = \log\left(\frac{p}{1-p}\right),
$$
and by "mlogit" we mean its multinomial extention (we suppress the formula here). 

## Predictions on the scale of $Y$

Predictions $\hat p$ on the scale of the response $Y$ are done by first calculating the value of the linear predictor $X\hat\beta$ and then applying the inverse link $g^{-1}$, i.e.
$$
  \hat p(X) = g^{-1}(X\hat \beta).
$$
These predictions are unbiased in the sense that their average on the data set used to fit the model equals the average response. *This important property only holds for the natural link function.* 

## Interpretation of effects

The interpretation of effects on the linear scale or for the identity link is as with linear regression: "A one-point increase in $X_m$ is associated with a $\beta_m$ increase in $g(E(y))$. Often, we want to be able to interpret the effects on $E(Y)$, i.e. with the equivalent model equation
$$
 E(y) = g^{-1}(X\beta)
$$
...
# data(dataCar)
# 
# barplot(table(dataCar$numclaims, useNA = "ifany"))
# 
# 
# barplot(table(dataCar$veh_body, useNA = "ifany"), horiz = TRUE)
# 
# 
# with(dataCar, table(clm, numclaims))
# 
# with(dataCar, hist(veh_value))
# 
# fit <- glm(clm ~ veh_value + veh_body + veh_age + gender + area + agecat, 
#            data = dataCar, family = binomial)
# summary(fit)
# 
