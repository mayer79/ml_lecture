...
As alternative to the multiple linear regression on diamond prices with logarithmic price and logarithmic carat, consider the same model without logarithms. Interpret the output of the model.
...
- **Using a log link produces a multiplicative model.**
...
- Predictions are on the right scale, e.g. probabilities of a binary response are between 0 and 1 if we use the logit link.
...
The interpretation of model coefficients is guided by the link.
...
- **Log link:** As with linear regression with log response: "A one-point increase in $X$ is associated with a relative increase in $E(Y)$ of $e^{\beta}-1 \approx \beta \cdot 100\%$. In For GLMs, this derivation is mathematically sound as we start with the logarithm outside the expectation. Using a GLM with log link is thus the cleaner way to produce a multiplicative model for $E(Y)$ than to log transform the response in a linear regression.
