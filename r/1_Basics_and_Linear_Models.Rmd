...
Covariables are often transformed before entering the model:

- Categorical covariables are dummy coded.
- Strongly correlated covariables might be decorrelated by forming ratios.
...
Since expectations and logarithms cannot be swapped, the calculation is not 100% correct. One consequence of this imperfection is that predictions backtransformed to the scale of $Y$ are systematically too small. One of the motivations of the generalized linear models GLM (see next section) will be to mend this problem in an elegant way.
...
Using logarithms for either price or carat did not provide a satisfactory model yet. What about applying logarithms to both response and covariable at the same time?
