...
fit <- glm(
  price ~ log(carat) + color + cut + clarity, data = dia, family = Gamma(link = "log")
)
summary(fit)
...
dia <- diamonds[, c(y, x)]
dia <- unique(dia)  #  Only new code line

# Split diamonds into 90% for training and 10% for testing
ix <- partition(dia[[y]], p = c(train = 0.9, test = 0.1), seed = 9838, type = "basic")
...
for (i in 1:nrow(paramGrid)) {
  ...
  for (fold in folds) {
    pred <- knn.reg(
      X_train[fold, ], test = X_train[-fold, ], k = k, y = y_train[fold]
    )$pred
...
for (fold in folds) {
  lm(price ~ log(carat) + color + cut + clarity, data = train[fold, ])
...
# Performance on test data
pred <- predict(fit, data.matrix(dataCar[ix$test, x]))
deviance_bernoulli(dataCar$clm[ix$test], pred)

# Relative performance
r_squared_bernoulli(dataCar$clm[ix$test], pred)
...
# As LightGBM does not understand strings, we encode them by integers
prep_lgb <- function(X, x = c("veh_value", "veh_body", "veh_age", 
                              "gender", "area", "agecat")) {
...
y_train <- dataCar$clm[ix$train]
X_train <- prep_lgb(dataCar[ix$train, ])

# LGB data handler. We could even set categorical_feature = "veh_body" in order
...
# Performance on test data
pred <- predict(fit, prep_lgb(dataCar[ix$test, ]))
deviance_bernoulli(dataCar$clm[ix$test], pred)

# Relative performance
r_squared_bernoulli(dataCar$clm[ix$test], pred) # 0.00445
# Relative performance gain is very low, but very slightly better than 
# of the tuned XGB model (could be the other way round).
...
fl <- flashlight(
  ...
  data = dataCar[ix$train, ], 
  label = "lgb", 
  predict_function = function(m, X) predict(m, prep_lgb(X))
)

light_profile(fl, v = "veh_value", breaks = seq(0, 5, by = 0.1)) %>% 
  plot() +
  ggtitle("Partial dependence plot for veh_value")

light_profile(fl, v = "veh_body") %>% 
  plot(rotate_x = TRUE) +
  ggtitle("Partial dependence plot for veh_body")

light_profile(fl, v = "area") %>% 
  plot() +
  ggtitle("Partial dependence plot for area")

light_profile(fl, v = "agecat") %>% 
  plot() +
  ggtitle("Partial dependence plot for agecat")
```
