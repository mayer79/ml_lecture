...
Using non-linear activation functions make sure that the network can represent non-linear terms and interactions. Typical activation functions are
...
1. Fit diamond prices by gamma deviance loss with log-link (i.e. exponential output activation), using the custom loss function defined below. Tune the model by simple validation and evaluate it on an independent test data set. Interpret the final model.
```{r}
