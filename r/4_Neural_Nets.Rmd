...
In this chapter, we dive into artificial neural networks. The first neural net was designed by Marvin Minsky in 1951. He called his algorithm SNARC ("stochastic neural-analog reinforcement calculator"). Modern neural networks are sometimes dated back to 1974 [1] to the dissertation of Paul J. Werbos, who had the idea to fit neural networks by the backpropagation algorithm, which is still a core element of modern neural networks.
...
Indeed: the solution of the simple neural network is quite similar to the OLS solution. 

## The optimization algorithm

Neural nets are fitted by **stochastic gradient descent (SGD)** resp. **backpropagation**. It works as follows:

1. Initiate the parameters with random values around 0. 
2. Forward step: Use the parameters to predict all observations of a *batch*. A batch is a randomly selected subset of the full data set. This makes the algorithm "stochastic".
3. Backpropagation step: Change the parameters in the right direction, making the loss of the current batch smaller. This involves calculating derivatives ("gradients") of the loss function (e.g. MSE) with respect to all parameters.
...
Oops, it seems as if the extra hidden layer had no effect. The reason is that a linear function of a linear function is still a linear function. Adding the hidden layer did not really change the model. It just added a lot of unnecessary parameters.
...
- binary logistic regression -> sigmoid activation (probability of "1")
- multinomial logistic regression -> softmax activation (one probability per class)
...
- Batch: A couple of data rows used for one SGD step.
- Callback: An action during training, e.g. saving weights, reducing learning rate or stop training.
- Epoch: The process of updating the network weights by SGD until each observation in the training set was used once.
...
- Stochastic gradient descent (SGD) resp. backpropagation: The optimization algorithm of neural networks.
- TensorFlow: An important implementation of neural networks.
- Weights: The parameters of a neural net.

# Chapter Summary

In this chapter, we made a glipse into the world of neural networks. Step by step we have learned how a neural network learns and why it automatically learns interactions and non-linear effects. We have used Keras and TensorFlow to build models brick by brick.
