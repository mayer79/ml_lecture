...
1. Fit diamond prices by gamma deviance loss with log-link (i.e. exponential output activation), using the custom loss function defined below. Tune the model by simple validation and evaluate it on an independent test data set. Interpret the final model. (Hints: I used a smaller learning rate of 0.03 and had to replace the "relu" activations by "tanh".)
```{r}
