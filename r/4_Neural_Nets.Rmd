...
- the linear rectangular unit "ReLU" $f(x) = \text{max}(0, x)$ that sets negative values to 0. Often used for large neural networks to save computational resources.
...
In order to show how to work with categorical embeddings (they are extremely useful).
We will now fit a neural net with two hidden layers (30 and 15 nodes) and a total of 631 parameters. Learning rate and batch size were chosen by validation data. The number of epochs is chosen by an early stopping callback.
...
prep_nn <- function(X, x_dense = x_dense, x_emb = x_emb, scaling = sc) {
  dense <- data.matrix(X[, x_dense, drop = FALSE])
  dense <- scale(dense, center = scaling$center, scale = scaling$scale)
  emb <- lapply(X[x_emb], function(x) as.integer(x) - 1)
  c(list(dense1 = dense), emb)
}
...
# Fit model - naive without validation
history <- nn %>% fit(
  x = prep_nn(X_train, x_dense = x_dense),
  y = y_train,
  epochs = 100,
  batch_size = 400, 
  validation_data = list(prep_nn(X_valid, x_dense = x_dense), y_valid),
  callbacks = cb
)
...
fl <- flashlight(model = nn, 
                 ...
                 predict_function = function(m, X) predict(m, prep_nn(X, x_dense = x_dense), 
                                                           batch_size = 1000))
...
plot(light_profile(fl, v = "veh_body")) +
  labs(title = "Partial dependence plot for clarity", y = "price")
...
1. Fit diamond prices by gamma loss (like a gamma GLM) with log-link, using the custom loss function defined below. Tune the model to achieve good performance.
```{r}
loss_gamma <- function(y_true, y_pred) {
  -k_log(y_true / y_pred) + y_true / y_pred
}
```
2. Fit a neural network on the claims data.
