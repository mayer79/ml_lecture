...
The first neural network was designed by Marvin Minsky in 1951. He called his algorithm SNARC ("stochastic neural-analog reinforcement calculator"). Modern neural networks are sometimes dated back to 1974 [1] to the dissertation of Paul J. Werbos, who had the idea to fit neural networks by the backpropagation algorithm, which is the gold standard as per today.
...
3. Backpropagation step: Change the parameters in the right direction, making the loss on the current batch smaller. This involves calculating derivatives ("gradients") of the loss function (e.g. MSE) with respect to all parameters.
...
Keras/TensorFlow offers many predefined loss functions and evaluation metrics. Choosing them is a crucial step.
Using TensorFlows backend functions, one can even define its own metrics (see example above for the root-mean-squared error) and loss functions (see the Exercises below).
...
1. Fit diamond prices by gamma loss with log-link, using the custom loss function defined below. Tune the model to achieve good performance. Interpret the model.
```{r}
loss_gamma <- function(y_true, y_pred) {
  -k_log(y_true / y_pred) + y_true / y_pred
}
```
2. Study either the optional claims data example or make your own neural net, predicting claim yes/no. For simplicity, you can represent the categorical feature `veh_body` by integers.
...
A summary and comparison of the algorithms we have met is on [github](https://github.com/mayer79/ML_Algorithm_Comparison). Here is a screenshot as per Sept. 7, 2020: ![](../figs/comparison_ML.PNG).
