...
3. Backpropagation step: Change the parameters in the right direction, making the average loss (e.g., the MSE) of the current batch smaller. This involves calculating derivatives ("gradients") of the loss function with respect to all parameters. Backpropagation does so in a layer-per-layer fashion, making heavy use of the chain rule. 
...
1. Fit diamond prices by minimizing Gamma deviance with log-link (-> exponential output activation), using the custom loss function defined below. Tune the model by simple validation and evaluate it (for simplicity) on the validation dataset. Interpret the final model. Hints: I used a smaller learning rate and had to replace the "relu" activations by "tanh". Furthermore, the response needed to be transformed from int to float.
