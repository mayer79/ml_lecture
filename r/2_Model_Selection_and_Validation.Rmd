...
6. Average the k model performances to estimate the model performance. The standard deviation gives an impression on the stability of that estimate.
...
In this chapter, we have met strategies to estimate model performance in a fair way. These strategies are also used for model selection and tuning. They are an essential part of the full modeling process. Often, ML models without appropriate validation strategy are not to be trusted. 

# Exercises

1. Apply *stratified* splitting (using the response `price`) throughout the last example. Compare the results.
2. Compare the linear regression model for `price` with a corresponding Gamma GLM with log-link. Use once (R)MSE for comparison and once Gamma deviance. What do you observe?
3. Use cross-validation to select the best polynomial degree to represent `log(carat)` in the linear regression model for `price`.
